...
public class TCPClient {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 1-3. SO_NODELAY(Nagle Algorithm off) true면 네이글 끔
			...
			os.write(data.getBytes("uft-8"));
			
			//5. 읽기
			byte[] buffer = new byte[256];
			int readByteCount = is.read(buffer); //blocking, 여기에 SoTimeout을 거는 것
			if(readByteCount == -1) {
				// 서버가 정상적으로 종료(명시적)(close() 호출)
